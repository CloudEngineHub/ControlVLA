...
from diffusion_policy.common.sampler import SequenceSampler, get_val_mask, ControlSequenceSampler
...
class ControlUmiDataset(BaseDataset):
    def __init__(self,
        shape_meta: dict,
        dataset_path: str,
        ...
        language_condition: Optional[str]=None,
    ):
        ...
        if cache_dir is None:
            ...
            import pickle
            # replay_buffer = pickle.load(open("./example_finetune_demo/cache/sortcube100_replay_buffer.pkl", 'rb'))
            with zarr.ZipStore(dataset_path, mode='r') as zip_store:
                replay_buffer = ReplayBuffer.copy_from_store(
                    src_store=zip_store, 
                    store=zarr.MemoryStore()
                )
            # pickle.dump(replay_buffer, open('./example_finetune_demo/cache/sortcube100_replay_buffer.pkl', 'wb'))
        else:
        ...
        control_shape_meta = shape_meta['control_obs']
        for key, attr in control_shape_meta.items():
            # solve obs type & only obs type current support
            type = attr.get('type', 'low_dim')
            if type == 'rgb':
                control_rgb_keys.append(key)
            else:
                raise RuntimeError('lowdim control obs type unsupported')
            horizon = control_shape_meta[key]['horizon']
            key_horizon[key] = horizon
            # solve latency_steps
            latency_steps = control_shape_meta[key]['latency_steps']
            key_latency_steps[key] = latency_steps
            # solve down_sample_steps
            down_sample_steps = control_shape_meta[key]['down_sample_steps']
            key_down_sample_steps[key] = down_sample_steps
        ...
        sampler = ControlSequenceSampler(
            shape_meta=shape_meta,
            replay_buffer=replay_buffer,
            rgb_keys=rgb_keys,
            lowdim_keys=self.sampler_lowdim_keys,
            control_rgb_keys=control_rgb_keys,
            control_lowdim_keys=control_lowdim_keys,
    ...
    def get_validation_dataset(self):
        ...
        val_set.sampler = ControlSequenceSampler(
            shape_meta=self.shape_meta,
            replay_buffer=self.replay_buffer,
            rgb_keys=self.rgb_keys,
            lowdim_keys=self.sampler_lowdim_keys,
            control_rgb_keys=self.control_rgb_keys,
            control_lowdim_keys=self.control_lowdim_keys,
    ...
    def get_updated_normalizer(self, normalizer: LinearNormalizer) -> LinearNormalizer:
        # update control obs normalizer
        for key in self.control_lowdim_keys:
            raise RuntimeError('lowdim control obs type unsupported')
        for key in self.control_rgb_keys:
            normalizer[key] = get_image_identity_normalizer()
        return normalizer
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        data_step_indices = self.sampler.indices[idx]

        # summary control obs dict
        control_obs_dict = dict()
        for key in self.control_rgb_keys:
            if not key in data:
                continue
            ...
            control_obs_dict[key] = data[key].astype(np.float32)
            # T,C,H,W
            del data[key]
        for key in self.control_lowdim_keys:
            raise RuntimeError('lowdim control obs type unsupported')
        ...
        torch_data = {
            ...
            'control_obs': dict_apply(control_obs_dict, torch.from_numpy),
            'action': torch.from_numpy(data['action'].astype(np.float32)),
            'text': self.language_condition,
            'step_indices': data_step_indices
        }
        return torch_data
