...
from diffusion_policy.common.sampler import DroidSequenceSampler, DroidSequenceSamplerNpy, get_val_mask
...
class DroidDataset(BaseDataset):
    def __init__(self,
        ...
        shuffle_neighbor_length: Optional[int]=300 * 500
    ):
        ...
        logger.debug(f'Dummy Val Mask for Debug')
        ...
        # load uniqe sampler for droid
        sampler = DroidSequenceSamplerNpy(
            shape_meta=shape_meta,
            datafolder=dataset_path,
            rgb_keys=rgb_keys,
        ...
        self.shuffle_neighbor_length = shuffle_neighbor_length
    ...
    def get_validation_dataset(self):
        ...
        val_set.sampler = DroidSequenceSamplerNpy(
    ...
    def shuffle(self):
        self.sampler.shuffle(neighbor_length=self.shuffle_neighbor_length)

    def __len__(self):
        return len(self.sampler) - self.start_global_step
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        try:
            data = self.sampler.sample_sequence(min(idx + self.start_global_step, len(self.sampler) - 1))

            obs_dict = dict()
            for key in self.rgb_keys:
                if not key in data:
                    continue
                # crop 180x320 --> 180x210 --> 224x224
                data[key] = data[key][:, :, 90:, :]
                # for debug, cv show raw and crop image
                obs_dict[key] = np.zeros((data[key].shape[0], 224, 224, 3), dtype=np.uint8)
                for i_t in range(data[key].shape[0]):
                    obs_dict[key][i_t] = cv2.resize(data[key][i_t], (224, 224), interpolation=cv2.INTER_CUBIC)
                # move channel last to channel first
                # convert uint8 image to float32
                obs_dict[key] = np.moveaxis(obs_dict[key], -1, 1).astype(np.float32) / 255.
                # T,C,H,W
                del data[key]
            ...
            torch_data = {
                'text': data['text'][np.random.randint(0, 3)],  ## default each episode has 3 language annotations
