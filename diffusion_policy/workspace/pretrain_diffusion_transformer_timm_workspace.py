...
# %%
class PretrainDiffusionTransformerTimmWorkspace(BaseWorkspace):
    ...
    def run(self):
        ...
        dataset.set_global_step(self.global_step * cfg.dataloader.batch_size * 4)  ## default 4 gpu
        assert isinstance(dataset, BaseImageDataset) or isinstance(dataset, BaseDataset)
        assert cfg.dataloader.shuffle == False, "Large-scale dataloader shuffled before each epoch"
        ...
        train_dataloader = DataLoader(dataset, collate_fn=collate_fn, **cfg.dataloader)

        # compute normalizer on the main process and save to disk
        # normalizer_path = os.path.join(self.output_dir, 'normalizer.pkl')
        if accelerator.is_main_process:
            logger.warning(f'normalizer is pre-computed in {cfg.task.dataset.dataset_path}')
            # normalizer = dataset.get_normalizer()
            # pickle.dump(normalizer, open(os.path.join(cfg.task.dataset.dataset_path, 'normalizer.pkl'), 'wb'))
            # exit()
            # normalizer = pickle.load(open(os.path.join(cfg.task.dataset.dataset_path, 'normalizer.pkl'), 'rb'))
            # if os.getenv('DEBUG', False):
            #     print("[debug] pre-load normalizer")
            #     # normalizer = dataset.get_normalizer()
            #     # pickle.dump(normalizer, open('example_demo_session/cache/normalizer.pkl', 'wb'))
            #     normalizer = pickle.load(open(os.path.join(cfg.task.dataset.dataset_path, 'normalizer.pkl'), 'rb'))
            # else:
            #     normalizer = dataset.get_normalizer()
        ...
        assert cfg.val_dataloader.shuffle == False, "Large-scale dataloader shuffled before each epoch"
        ...
        logger.warning(f"Logging to {log_path}")
        with JsonLogger(log_path) as json_logger:
            for local_epoch_idx in range(cfg.training.num_epochs):
                self.model.train()
                
                # Manually shuffle the dataset
                if cfg.training.shuffle_dataset_manually:
                    logger.info("Shuffling dataset within the neighborhood")
                    dataset.shuffle()
                    train_dataloader = DataLoader(dataset, collate_fn=collate_fn, **cfg.dataloader)
                ...
                with tqdm.tqdm(train_dataloader, desc=f"Training epoch {self.epoch}", 
                    ...
                    for batch_idx, batch in enumerate(tepoch):
                        ...
                        try:
                            ...
                            logger.error(f"Error transferring batch {batch_idx} to device: {e}")
                            continue
                        ...
                        if self.global_step % cfg.training.checkpoint_every_step == 0:
...
def main(cfg):
    workspace = PretrainDiffusionTransformerTimmWorkspace(cfg)
    workspace.run()
