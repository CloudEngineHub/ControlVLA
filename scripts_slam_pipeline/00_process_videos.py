...
def main(session_dir):
    ...
    for session in session_dir:
        ...
        # hardcode subdirs
        # print(session, pathlib.Path(os.path.expanduser(session)))
        ...
        if not gripper_cal_dir.is_dir():
            ...
            with ExifToolHelper() as et:
                print(list(input_dir.glob('**/*.MP4')) + list(input_dir.glob('**/*.mp4')))
                for mp4_path in list(input_dir.glob('**/*.MP4')) + list(input_dir.glob('**/*.mp4')):
                    if mp4_path.name.startswith('map'):
                        continue
                    
                    start_date = mp4_get_start_datetime(str(mp4_path))
                    print(f"MP4 path: {mp4_path} | Start date: {start_date}")
        ...
        with ExifToolHelper() as et:
            print(f'mp4_path: {input_mp4_paths}')
            for mp4_path in input_mp4_paths:
                ...
                try:
                    start_date = mp4_get_start_datetime(str(mp4_path))
                    print(f'mp4data: {mp4_path} | Start date: {start_date}')
                    # continue
                except Exception as e:
                    import datetime
                    import random
                    start_date = datetime.datetime(2021, 2, 1) + datetime.timedelta(days=1, seconds=random.randint(0, 86399))
                    print(f"Error mp4data: {mp4_path} | random a datatime : {start_date}")
                    # continue
                meta = list(et.get_metadata(str(mp4_path)))[0]
                print(meta)
                try:
