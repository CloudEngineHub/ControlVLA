    ...
    for demo_idx, demo_data in enumerate(demo_data_list):
        ...
        for vid_idx in video_idxs:
            ...
            if csv_df['is_lost'].sum() > 10:
                ...
                # unreliable tracking
                print(f'Excluded demo {demo_idx} due to too many lost frames.')
                break
            
            if (~csv_df['is_lost']).sum() < 60:
                print(f'Excluded demo {demo_idx} due to too few valid frames.')
                break
        ...
        if len(pose_interps) != n_gripper_cams:
            # invalid episode
            print(f"Excluded demo {demo_idx} from left/right disambiguation., pose_interps={len(pose_interps)} | n_gripper_cams={n_gripper_cams}")
            continue
