...
def main(input, output, out_res, out_fov, compression_level, 
    ...
    for ipath in input:
        ...
        # logger.warning(f'down sample 5 episodes for debug')
        # plan = plan[:2]
    ...
    for cam_id in range(n_cameras):
        ...
        name = f'camera{cam_id}_rgb_narrow'
        _ = out_replay_buffer.data.require_dataset(
            name=name,
            shape=(out_replay_buffer['robot0_eef_pos'].shape[0],) + out_res + (3,),
            chunks=(1,) + out_res + (3,),
    ...
    def video_to_zarr(replay_buffer, mp4_path, tasks):
        ...
        name = f'camera{camera_idx}_rgb_narrow'
        img_array_narrow = replay_buffer.data[name]
        ...
        with av.open(mp4_path) as container:
            ...
            for frame_idx, frame in tqdm(enumerate(container.decode(in_stream)), total=in_stream.frames, leave=False):
                ...
                if frame_idx < tasks[curr_task_idx]['frame_start']:
                    ...
                    img_narrow = copy.deepcopy(img)
                    # mask out gripper
                    img = draw_predefined_mask(img, color=(0,0,0), 
                        mirror=no_mirror, gripper=True, finger=False)
                    img_narrow = draw_predefined_mask(img_narrow, color=(0,0,0),
                        mirror=True, gripper=False, finger=False)
                        
                    # resize
                    # img_narrow = resize_tf(img_narrow)
                    ...
                    # narrow view construction
                    img_narrow = narrow_fisheye_converter.forward(img_narrow)
                        
                    # compress image
                    img_array[buffer_idx] = img
                    img_array_narrow[buffer_idx] = img_narrow
                    buffer_idx += 1
