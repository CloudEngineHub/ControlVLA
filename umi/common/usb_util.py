...
def reset_all_elgato_devices():
    ...
    for dev in device_list:
        if 'UGREEN' in dev['description'] or 'UGREEN' in dev['device']:
            dev_usb_path = dev['path']
            reset_usb_device(dev_usb_path)
        elif 'Elgato' in dev['description'] or 'Elgato' in dev['device']:
            dev_usb_path = dev['path']
            reset_usb_device(dev_usb_path)
        elif 'Actions Microelectronics Co.' in dev['description']:
            dev_usb_path = dev['path']
            reset_usb_device(dev_usb_path)
