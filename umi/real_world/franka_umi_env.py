...
from umi.real_world.franka_deoxys_controller import FrankaDeoxysController
...
class FrankaUmiEnv:
    def __init__(self, 
            ...
            # shared memory
            shm_manager=None,
            franka_deoxys_controller_frequency=200,
            ...
            if 'Cam_Link_4K' in path:
                ...
                def tf(data, input_res=res):
                    ...
                    if fisheye_converter is None:
                        ...
                        img = fisheye_converter.forward(f(img))
                        img = img[...,::-1]
                    if obs_float32:
                        img = img.astype(np.float32) / 255
                    data['color'] = img
        ...
        robots: List[FrankaDeoxysController] = list()
        ...
        for rc in robots_config:
            if rc['robot_type'].startswith('ur5'):
                ...
                this_robot = FrankaDeoxysController(
                    shm_manager=shm_manager,
                    # robot_ip=rc['robot_ip'],
                    frequency=franka_deoxys_controller_frequency,
                    Kx_scale=1.0,
        ...
        self.robots: List[FrankaDeoxysController] = robots
    ...
    def exec_actions(self, 
        ...
        for i in range(len(new_actions)):
            for robot_idx, (robot, rc) in enumerate(zip(self.robots, self.robots_config)):
                ...
                g_latency = rc['gripper_action_latency'] if compensate_latency else 0.0
