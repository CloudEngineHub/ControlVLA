...
class SharedMemoryRingBuffer:
    ...
    def get_last_k(self, k:int, out=None) -> Dict[str, np.ndarray]:
        if k > self.get_max_k:
            logger.warning(f'Accesing shared memory, but asked for more than buffer size: asked for {k} but limit is {self.get_max_k}')
            k = self.get_max_k
        if out is None:
        ...
        try:
            ...
            # logger.exception("Assertion failed in worker process")
